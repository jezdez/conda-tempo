...
| **Date** | April 24, 2026 |
| **Status** | B22 merged 2026-09-09 after its final 94-check CI matrix passed. Across the original 22 implementation PRs, eight non-draft PRs are green and mergeable, while B30 remains a mergeable draft with CI failures. Status refreshed 2026-09-10. The W3@50k experimental stack measured mac 12.4 s / Linux 8.0 s (>24× / >37× vs intractable baseline) |
...
- [B31: Preserve lazy indexes during conda-build solves](#b31-preserve-lazy-indexes-during-conda-build-solves)
...
> _Last refreshed 2026-09-10 to record B22's merge and audit the remaining
> Track B PRs. B22's cache-selection measurement was refreshed on 2026-09-09.
> B31's core and solver changes merged on 2026-09-08, while the conda-build
> fix awaits review. Headline macOS/Linux end-to-end numbers remain unchanged
> since the 2026-04-24 stacked runs. B22 and B31 have separate focused
> measurements._

**Experimental-stack result: ~10–20 % faster on typical installs,
20–40× faster on commands against large existing prefixes.** These
totals measure the April 2026 full experimental branch stack, not the
current eight-merged-PR post-solver subset. That stack included B4, B14, and B20,
which have not all shipped. The merged subset has not been remeasured
end to end, so these totals are not current conda performance. The
20–40× result is the user-visible story (``conda update --all`` on a
long-lived research env went from intractable to ~10 s). The 10–20 %
result is the steady background improvement. Everything below is
measured wall time from hyperfine on macOS (M1 Pro / APFS) and arm64
Linux (OrbStack / ext4)
against stock conda on the same hardware; x86_64 Linux is not yet
measured. Windows x86_64 checkpoint data exists in
[`data/phase{1,2,4}_windows/`](data/) plus the follow-up W3
topological-sort probe in
[`data/phase4_windows_toposort_probe/`](data/phase4_windows_toposort_probe/),
but is tracked separately because the available July 2026 branch stack
did not reproduce the April macOS/Linux stacked behavior.
...
|---|---|---|---|---|
| B1 | conda | Quadratic diff sort | 782× at N=50 000 | [conda/conda#15970](https://github.com/conda/conda/pull/15970) merged 2026-07-11 |
| B2 | conda | O(N²) `PrefixGraph.__init__` | 53× at N=1 000 | [conda/conda#15971](https://github.com/conda/conda/pull/15971) merged 2026-07-12 |
| B4 | conda | `sha256_in_prefix` gated on `extra_safety_checks` | 27 % per-file at 1/10/50 MB, but only by dropping `conda doctor` altered-file data | [conda/conda#15972](https://github.com/conda/conda/pull/15972) is green and mergeable after its 2026-09-01 rebase, with no unresolved review threads. Recommend closing unless altered-file detection becomes opt-in |
| B6 | conda | Parallel transaction-action verification through the shared executor | Current-main real-package runs: K=2 was 14-36 % slower and K=4 was 14-40 % slower in prepare/verify | [conda/conda#15973](https://github.com/conda/conda/pull/15973) closed; B9c removed the per-action codesign cost that had justified this direction |
| B7 | conda | Parallel `posix.link` fan-out | 1.52× on mac, **regresses on Linux** | dropped |
| B8 | conda | Process-pool package extraction | 1.56× vs serial on the S8 fresh-cache fixture | [conda/conda#15974](https://github.com/conda/conda/pull/15974) merged 2026-07-14 |
...
| B9c | conda | Codesign batching for osx-arm64 rewrites | W1 mac −33 %, W2 mac −15 % | [conda/conda#15975](https://github.com/conda/conda/pull/15975) merged 2026-07-11 |
| B11 | conda-libmamba-solver | Cache `SolverInputState.installed` | 6500× per-access | [conda/conda-libmamba-solver#921](https://github.com/conda/conda-libmamba-solver/pull/921) merged 2026-07-11 |
...
| B13 (cph) | cph | Thread one `ZipFile` through both components (depends on cps#173) | 2× per archive | [conda/conda-package-handling#318](https://github.com/conda/conda-package-handling/pull/318) merged |
| B14 | cps | Skip `utime` in `TarfileNoSameOwner` | 3.4 % per extract, but the mtime premise was wrong | [conda/conda-package-streaming#174](https://github.com/conda/conda-package-streaming/pull/174) closed and dropped |
...
| B20 | cps | Hybrid fast/fallback per-member safety check | +22.6 % Linux, neutral mac | [conda/conda-package-streaming#175](https://github.com/conda/conda-package-streaming/pull/175) is green and mergeable, with one current and two outdated unresolved review threads and no reply to the 2026-06-30 follow-up |
...
| B22 | conda | Prefer same-device package cache entries | 2026-09-09 cross-device fixture: 165.76 → 97.41 ms, **1.70×**, by selecting same-device hardlinks. [Method and raw samples](#b22-same-device-cache-selection) | [conda/conda#16347](https://github.com/conda/conda/pull/16347) merged 2026-09-09 from `6137fcd` after all review threads were resolved and all 94 checks passed |
...
| B28 | conda | Linux `FICLONE` for copy-mode installs | btrfs copy-mode file creation with 64 KiB gate: 64 MiB file 242×, 512 small files 1.54×, tiny files avoid raw ioctl path | [conda/conda#16367](https://github.com/conda/conda/pull/16367) is green and mergeable on current B27 after its 2026-09-01 restack, with no unresolved review threads and approval pending |
...
| B30 | conda | Clone eligible package subtrees on APFS | W2 −22.7 %, add-to-existing-prefix −26.6 % versus B28 | [conda/conda#16376](https://github.com/conda/conda/pull/16376) is a mergeable draft on B28 with seven unresolved review threads. Its latest run has 91 successful and three failed checks, comprising two macOS test jobs and the derived `analyze` job |
| B31 | conda-build + conda-libmamba-solver | Preserve lazy indexes during conda-build solves | At 1M records, 2,161.433 → 0.163 MiB peak tracked heap in the isolated methods | Core [conda/conda#16635](https://github.com/conda/conda/pull/16635) and solver [conda/conda-libmamba-solver#1044](https://github.com/conda/conda-libmamba-solver/pull/1044) merged 2026-09-08. [conda/conda-build#6126](https://github.com/conda/conda-build/pull/6126) remains open and awaiting review, tracked by [#6125](https://github.com/conda/conda-build/issues/6125) |

B31 and its implementation PRs are tracked separately from the dated filed-PR counts below. Its
...
The live 2026-09-10 audit supersedes that snapshot. B22 merged on 2026-09-09
after its final 94-check matrix passed. All eight remaining open non-draft PRs
in the original set are green and mergeable, with approval pending. B30 is
mergeable but remains a draft after two macOS test jobs and the derived
`analyze` job failed. B31's core and solver PRs merged on 2026-09-08, while its
conda-build PR awaits review.
...
- **PrefixGraph on realistic large-prefix installs is still the next
  big opportunity, but B15 is the wrong implementation.** The
  ...
  platform edge cases. The 2026-07-02 Windows W3 follow-up found a
  narrower pure-Python opportunity first: on B2+B11, a 10k realistic
  W3 profile spent **52.3 s cumulative in `PrefixGraph.__init__`**,
  almost all of it in `_toposort()`, and a local child-adjacency
  ...
  keeps a custom Kahn-style queue with conda's deterministic ordering
  and cycle-handling safeguards. A focused synthetic topo comparison
  showed stdlib `graphlib.TopologicalSorter` is still far faster than
  the original repeated-scan path, but slower than the custom queue. The
  current custom PR rerun measured Windows W3 realistic 50k at **64.46 s**
  on B2+B11+B21. Future rattler work should be a wider first-class record/spec
  architecture question, not a narrow optional fast path inside
  `PrefixGraph.__init__`.
- **Windows is now partially measured, not final-stack validated.**
  ...
  2026-07-02 topological-sort probe and PR branch confirmation, but it
  is not a like-for-like replacement for the April macOS/Linux
  full-stack table. `menuinst`'s per-shortcut NTFS/AV cost remains a
  known gap.

### Next steps

As of 2026-09-10, eight of the original 22 implementation PRs are merged. Eight
non-draft PRs remain open. All are green and mergeable, but none is approved.
B30 is the only draft. It is mergeable, but its latest CI run failed. B6, B14,
B15, B23, and B29 are closed. The nearest-term work is:
...
2. Get reviewer follow-up on B21 [#16331](https://github.com/conda/conda/pull/16331), B24
   ...
   [#16353](https://github.com/conda/conda/pull/16353). All three are green and
   mergeable, with all prior review threads resolved.
3. Review and land the platform copy stack in order: B26
   ...
   and mergeable. B28 is also green and mergeable after its 2026-09-01 restack
   on current B27. B27 has no formal review yet.
4. Fix B30's failing macOS tests
   [#16376](https://github.com/conda/conda/pull/16376), then resolve its seven
   review threads. The current failures cover softlink preservation and package
   cache cleanup behavior.
5. Decide whether B4 [#15972](https://github.com/conda/conda/pull/15972) should
   make altered-file detection opt-in. Rebase it if yes, otherwise close it.
6. Get reviewer follow-up on the B31 conda-build fix
   [conda-build#6126](https://github.com/conda/conda-build/pull/6126) and complete
   the recipe-build measurement.
7. Add Linux x86_64 numbers and rerun the Windows W3/W4 stack against an exact
   or equivalent April stack. The current Windows data is useful but not a
   like-for-like replacement for the macOS/Linux full-stack runs.
...
Two explicitly scoped solver-preparation follow-ups are also tracked here:
B11 caches installed solver state, and B31 preserves lazy indexes during
conda-build solves. B31 extends the workspace to conda-build's Python index
handoff and its conda-libmamba-solver consumer. General recipe-building work
remains outside Track B. These measurements stay separate from post-solver
and historical end-to-end totals.
...
- General package-building work in `conda-build`, `conda-smithy`, and `boa`.
  B31's conda-build solver preparation is an explicit exception.
...
- Other `conda-libmamba-solver` pre-solve / repodata loading. B11's installed
  state caching and B31's lazy-index preservation are explicit exceptions.
  Fixes remain in the repository that owns the affected code.
...
(later opened as the draft PR [conda/conda#15980](https://github.com/conda/conda/pull/15980)
after W5 validated the seam end-to-end), stacked on B2. New
`conda/models/_prefix_graph_rattler.py` helper
...
The original April projection for W1 (Phase-1 cProfile showed 200 actions ×
4.97 s = 24.9 ms/action at real package file sizes — larger than our 4 KB
...
other fix. This projection is superseded by the current-main remeasurement
below: it attributed work later removed from individual actions by B9c and did
not include executor scheduling for the much larger population of cheap path
checks.

##### S6 current-main remeasurement and disposition (2026-07-20)

Re-ran [#15973](https://github.com/conda/conda/pull/15973) at `34a5720`
against conda `main` at `0cdbbf6`, after B9c
[#15975](https://github.com/conda/conda/pull/15975) merged. Runs used real
conda-forge packages, warm package caches, randomized paired ordering, and the
real solve plus transaction prepare/verify path. The macOS W1 and W2 workloads
used 15 and 10 repetitions; Linux arm64 W1 used 10 repetitions.

| Workload | K=2 vs PR K=1 prepare/verify | K=4 vs PR K=1 prepare/verify | Solve + prepare/verify wall, K=2 / K=4 |
|---|---:|---:|---:|
| macOS W1, 29 packages | **+22.7 %** (95 % median CI +20.8 to +26.1 %) | **+28.0 %** (+25.4 to +30.3 %) | **+13.1 % / +15.3 %** |
| macOS W2, 211 packages | **+35.6 %** (+32.7 to +37.7 %) | **+39.5 %** (+36.5 to +41.8 %) | **+24.9 % / +27.3 %** |
| Linux arm64 W1, 35 packages | **+14.1 %** (+12.0 to +15.8 %) | **+14.4 %** (+9.6 to +17.7 %) | **+6.2 % / +2.6 %** |

The action profile explains the regression. W1 submitted 8,080 actions and W2
submitted 36,811 actions, but 97-98 % were ordinary `LinkPathAction` checks
with a median cost of about 24 µs. Only 202/364 were prefix rewrites, with
medians of 0.48/0.57 ms. Mapping every action through the executor costs more
than the work it overlaps. The PR's four-action test sleeps 50 ms per action,
so it proves concurrency against a workload roughly 100 times heavier than a
real prefix rewrite and about 2,000 times heavier than the dominant path
check.

A 200-action prefix-rewrite crossover probe found that K=2 only began helping
around 256 KiB files (1.08x) and reached 1.15x at 1 MiB; K=4 was slower at
every tested size. Real prefix-rewrite files had 1.4-1.5 KiB medians and
113-222 KiB p95 sizes. Enabling `extra_safety_checks` also remained slower on
warm caches. **Disposition: close B6.** A future attempt would need to batch
work or select only demonstrably expensive actions, with fresh end-to-end
evidence.
...
| **B6** | `conda/conda:jezdez/track-b-b6-verify-parallel` | Historical synthetic: 1.26× at K=2; current-main real-package runs: 14-36 % slower | Closed; executor overhead dominates after B9c |
...
| B4 | S4 | Gate ``sha256_in_prefix = compute_sum(...)`` on ``context.extra_safety_checks``. Phase-2 data showed a **27 % per-file verify reduction** at 1/10/50 MB, but the implementation removes the reference digest used by `doctor.health_checks.altered_files`. Computing the digest during prefix replacement was slower than the current two-pass implementation. The PR conflicts with `main` and should close unless altered-file detection is intentionally made opt-in. |
...
| B6 | S6 | **CLOSED.** Current-main real-package runs on macOS and Linux made prepare/verify 14-40 % slower with K=2/K=4. B9c moved codesign out of each action, leaving thousands of ~24 µs checks whose executor overhead dominates. The original synthetic test and PR test used much heavier actions and did not represent the shipping action mix. |
...
| **B14** | extract utime no-op (cps-level) | **DROPPED.** The 3.4 % extraction win was real, but the premise was not. `conda-package-handling` does not canonicalize tar member mtimes, so skipping `utime` changes extraction semantics. [conda-package-streaming#174](https://github.com/conda/conda-package-streaming/pull/174) was closed. |
...
The surviving April items are independent. B6 and B7 are both closed/dropped;
neither gates the remaining work.
...
Live PR statuses were refreshed on 2026-09-10.
...
| B22 | Prefer same-device package cache entries | [conda/conda#16347](https://github.com/conda/conda/pull/16347) merged 2026-09-09 from `6137fcd` after all review threads were resolved and all 94 checks passed. |
...
| B28 | Use Linux `FICLONE` for copy-mode installs | [conda/conda#16367](https://github.com/conda/conda/pull/16367) is green and mergeable on current B27 after its 2026-09-01 restack, with no unresolved review threads and approval pending. |
...
| B30 | Clone eligible package subtrees on APFS | [conda/conda#16376](https://github.com/conda/conda/pull/16376) is a mergeable draft on B28 with seven unresolved review threads. Its latest run has 91 successful and three failed checks, comprising two macOS test jobs and the derived `analyze` job. |
...
| B24 | 2,000 Python files with `compile_pyc: false` | 1.03 s compile phase | skipped | n/a |
| B24 | 2,000 Python files, `pyc_compile_threads: 2` versus B23 helper | 1.22 s | 1.14 s | 1.07× |
| B24 | 2,000 Python files, `pyc_compile_threads: 4` versus B23 helper | 1.22 s | 1.24 s | 0.98× |
| B25 | no-failure extraction wrapper, simulated Windows path | 0.10 us/call | 0.31 us/call | n/a |
| B25 | 4 transient `EACCES` failures before success | fails after 1 attempt | succeeds on attempt 5 | n/a |
...
| B27 | 1 copy-mode file, 64 MiB on Windows NTFS | 44.02 ms | 16.18 ms | 2.72× |
| B27 | 512 copy-mode files, 64 KiB each on Windows NTFS | 1.897 s | 186.35 ms | 10.18× |
| B27 | 2,048 copy-mode files, 1 KiB each on Windows NTFS | 8.311 s | 944.57 ms | 8.80× |
| B28 | 1 copy-mode file, 64 MiB on Linux btrfs, 64 KiB gate | 14.47 ms | 0.060 ms | 242× |
| B28 | 512 copy-mode files, 64 KiB each on Linux btrfs, 64 KiB gate | 22.41 ms | 14.58 ms | 1.54× |
| B28 | 2,048 copy-mode files, 1 KiB each on Linux btrfs, 64 KiB gate | 53.34 ms | 55.71 ms | 0.96× |
| B29 | Powered W1, normal hardlink install | 5.915 s | 5.919 s | neutral, PR closed |
| B30 | W1 fresh Python + requests | 6.022 s | 5.939 s | 1.4 % faster |
...
B27's powered 20-repeat run also compared `CopyFile2` with `CopyFileW` on the
same Python 3.13.13 NTFS fixture. Follow-up 80-repeat probes on the same VM did
not reproduce the initial 7.1 % single-file difference. API-only timing put
`CopyFile2` 0.5 % faster, while the original destination and `copystat` shape
put it 0.9 % slower. Treat the native APIs as performance parity within about
1 % on this fixture.

B28's raw size sweep on Ubuntu btrfs shows the threshold pattern behind
...
The first better Windows W3 50k strategy was: do not spend a five-run
...
The 50k realistic fixture was still much slower than the April
macOS/Linux full-stack result, but it was a successful bounded
measurement instead of a blind timeout.

A follow-up Windows profile on the B2+B11-aligned 10k realistic path
showed the remaining gap was not primarily `MatchSpec`. The profile in
[`data/phase4_windows/w3_b2b11_realistic_10k/cprofile.top20.txt`](data/phase4_windows/w3_b2b11_realistic_10k/cprofile.top20.txt)
spent **52.3 s cumulative in `PrefixGraph.__init__`**, **51.0 s** in
`_toposort()`, and made **87.0 M** `dict.pop()` calls. A local
measurement prototype replaced `_toposort_raise_on_cycles()` with a
child-adjacency Kahn pass; the exact prototype patch is recorded in
[`data/phase4_windows_toposort_probe/prefix_graph_toposort_prototype.patch`](data/phase4_windows_toposort_probe/prefix_graph_toposort_prototype.patch).
It is not yet a production PR because the cycle-handling path needs
tests that prove `_topo_sort_handle_cycles()` sees the same remaining
graph shape after acyclic nodes are removed.

With that prototype, the same Windows host produced:

| Probe | B2+B11 | B2+B11 + local toposort prototype |
|---|---:|---:|
| W3 realistic, 5k records | 9.25 s | 5.63 s |
| W3 realistic, 10k records | 27.64 s | 9.54 s |
| W3 realistic, 50k records | 574.61 s | 33.23 s |

Raw data:
[`data/phase4_windows_toposort_probe/w3_strategy/realistic.json`](data/phase4_windows_toposort_probe/w3_strategy/realistic.json),
[`data/phase4_windows_toposort_probe/w3_b2b11_toposort_10k/cprofile.top20.txt`](data/phase4_windows_toposort_probe/w3_b2b11_toposort_10k/cprofile.top20.txt),
and
[`data/phase4_windows_toposort_probe/pstats_focus.txt`](data/phase4_windows_toposort_probe/pstats_focus.txt).
In the patched 10k profile, `PrefixGraph.__init__` falls from **52.3 s**
to **1.3 s** cumulative and `dict.pop()` falls from **87.0 M** calls to
**153 k** calls. The remaining top frame is no longer graph sorting; it
is libmamba metadata/index setup, `History.get_requested_specs_map()`,
and `PrefixData.load()`.

Production PR [conda/conda#16331](https://github.com/conda/conda/pull/16331)
keeps the local child-adjacency Kahn pass rather than using stdlib
`graphlib.TopologicalSorter`, while preserving conda's deterministic
per-level ordering and the remaining-graph behavior that
`_topo_sort_handle_cycles()` relies on. The standalone B21 PR is now at
`f1abfcbd`; the B2+B11+B21 measurement branch used `0ec431797` for the
combined stack because the W3 bottleneck is visible only after B2 removes
the larger graph-construction cost. A focused synthetic topo comparison
on Windows, using a chain-like graph where each node depends on the
previous three nodes, measured:

| Records | Original repeated scan | `graphlib` | Custom Kahn pass |
|---:|---:|---:|---:|
...
So `graphlib` remains far faster than the original repeated-scan path,
but it is slower than the custom queue. The current custom implementation
then reran the B2+B11+B21 Windows strategy and produced:

| Probe | B2+B11 | B2+B11 + #16331 |
|---|---:|---:|
| W3 realistic, 5k records | 9.25 s | 8.02 s |
| W3 realistic, 10k records | 27.64 s | 13.74 s |
| W3 realistic, 50k records | 574.61 s | 64.46 s |

Raw data:
[`data/phase4_windows_toposort_pr_custom/w3_strategy/realistic.json`](data/phase4_windows_toposort_pr_custom/w3_strategy/realistic.json).
...
is open, with its handoff fix awaiting review in
[#6126](https://github.com/conda/conda-build/pull/6126). The solver-side change
[conda/conda-libmamba-solver#1044](https://github.com/conda/conda-libmamba-solver/pull/1044)
merged on 2026-09-08. Its release tracking issue
[#1045](https://github.com/conda/conda-libmamba-solver/issues/1045) remains open
for 26.9.0. Core [conda/conda#16635](https://github.com/conda/conda/pull/16635)
also merged on 2026-09-08, and [#16638](https://github.com/conda/conda/issues/16638)
is closed.

The conda-build handoff evaluates `if index`, then copies the index into
`solver._index`. Both operations independently access `Index.data` before
`solve_for_transaction()` runs. An offline reproduction with the actual
conda-build callable and a mock solver backend confirms both accesses.
The [current source](https://github.com/conda/conda-build/blob/d4f1b0302e710d1eb7e106cc5c0557f0213b62e9/conda_build/environ.py#L1407-L1421)
still contains both operations. Changing the truthiness check alone is insufficient.

The solver PR removes two further eager operations from
...
available in supported conda versions. Merged [conda/conda#16635](https://github.com/conda/conda/pull/16635)
...
| --- | --- | --- |
| Eager truthiness and copying added to conda-build | [conda-build#5074](https://github.com/conda/conda-build/pull/5074), merged January 26, 2024, copied from conda's existing `install_actions` | [conda-build 24.1.0](https://github.com/conda/conda-build/releases/tag/24.1.0), February 1, 2024 |
| Lazy `Index` added to conda | [conda#13880](https://github.com/conda/conda/pull/13880), merged September 20, 2024 | [conda 24.9.0](https://github.com/conda/conda/releases/tag/24.9.0), September 30, 2024 |
| Conda-build adopted lazy `Index` but retained both eager operations | [conda-build#5488](https://github.com/conda/conda-build/pull/5488), merged October 2, 2024 | [conda-build 24.11.0](https://github.com/conda/conda-build/releases/tag/24.11.0), November 20, 2024 |
...
Comparison: conda-libmamba-solver original
`84de85e38f7009634c29b0162faeb060af0b5891` and PR head
`33498b5d1d3fbf7c4c822fd7c5419e9268d483e4`. Runtime: Python 3.14.7,
conda 26.7.1, conda-build 26.7.1, libmambapy 2.9.0, Memray 1.20.0,
macOS arm64.

Each value is the median of three fresh sequential processes, alternating
revision order. Input is identical generated local repodata. The large
channel has the displayed record count, with one additional package in the
output channel. The measured sequence calls build detection, then channel
discovery, which invokes detection again internally.

| Large-channel records | Original Memray peak (MiB) | PR Memray peak (MiB) | Original process peak RSS (MiB) | PR process peak RSS (MiB) |
|---:|---:|---:|---:|---:|
| 10,000 | 44.415 | 0.163 | 166.64 | 97.39 |
| 100,000 | 239.167 | 0.163 | 465.84 | 97.84 |
| 1,000,000 | 2,161.433 | 0.163 | 3,269.58 | 98.73 |

Memray reports additional peak tracked live heap during the calls, using
normal Python allocator behavior. RSS comes from separate unprofiled runs.
Both revisions return the same output channel with its platform removed.
The original realizes every record, while the PR realizes none.

These figures include the repodata parsing that the original methods trigger.
The cold local file transport also loads roughly 23 MB of encoding-detection
models, which is fixed overhead rather than PackageRecord storage. The inputs
are synthetic local channels, not conda-forge, and the figures are not complete
recipe-build measurements.

### Already-realized control and completion criteria

With the 100,000-record index realized before either revision runs, the
original retains a median 474.84 MiB process peak RSS and the PR retains
468.89 MiB. Their ranges overlap, and both record 0.163 MiB of additional
peak tracked heap. The earlier conda-build allocation therefore needs its
own fix before claiming the larger memory saving from the solver PR.

Completion requires the conda-build handoff fix and the related solver fix,
regression tests guarding unwanted `Index.data` access, and a complete recipe
build measured with the coordinated changes. The complete-build measurement
is still outstanding. These method-level results do not change Track B's
existing post-solver or end-to-end performance totals.

[Individual measurements, medians, and input hashes](data/phase2/lazy_index/2026-09-07/)
and the [benchmark harness](bench/phase2/lazy_index/README.md) preserve the
methodology and reproduction instructions. Raw captures and diagnostic logs
...
| Date | Change |
|---|---|
| 2026-09-10 | **Live Track B status refreshed after B22 merged.** [conda/conda#16347](https://github.com/conda/conda/pull/16347) merged on 2026-09-09 from `6137fcd` after all review threads were resolved and all 94 checks passed. Eight of the original 22 implementation PRs are now merged. All eight remaining non-draft PRs are green and mergeable, with approval pending. B30 is a mergeable draft whose latest run has 91 successful checks, two failed macOS test jobs, and the derived failed `analyze` job. B31 core [conda/conda#16635](https://github.com/conda/conda/pull/16635) and solver [conda/conda-libmamba-solver#1044](https://github.com/conda/conda-libmamba-solver/pull/1044) also merged on 2026-09-08, while [conda/conda-build#6126](https://github.com/conda/conda-build/pull/6126) awaits review. Measurements are unchanged. |
